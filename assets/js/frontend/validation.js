...
function checkBoxValidationAdmin(req,AdmEmail) {
	...
	if(tot == 0) {
			sweetAlert("Oops...", "Please Select the CheckBox", "error");
			return false;
	}else if(chkVal == 'on') {
			sweetAlert("Oops...", "No records found", "error");
			return false;
...
function checkBoxWithSelectValidationAdmin(req,AdmEmail) {
	...
	if(templat==''){
		sweetAlert("Oops...", "Please select the mail template", "error");
		return false;
	}
	var tot=0;
	var chkVal = 'on';
...
function bulk_logs_action(req,AdmEmail){
	...
	if(perms==''){
			sweetAlert("Oops...", "Please Enter The Email ID", "error");
			return false;
	}else if(perms==null){
			return false;
	}else{
		if(perms==AdmEmail){
				...
				sweetAlert("Oops...", "Please Enter The Correct Email ID", "error");
				return false;
		}
	}
...
function checkBoxCategory() {
	...
	for (var i = 0; i < frm.length; i++){
		...
		if(tot == 0) {
				...
				sweetAlert("Oops...", "Please Select only one CheckBox at a time", "error");
				return false;
		}else if(chkVal == 'on') {
				sweetAlert("Oops...", "No records found", "error");
				return false;
	...
	$.ajax(
		    {
		        ...
		        {
		        	if(response == 'No') {
						sweetAlert("Oops...", "You can't delete all the images", "error");
						return false;
					  } else {
							  $('#img_'+val).remove();
					  }
...
function quickSignup(){
	...
	$.ajax({
        type: 'POST',
        url: baseURL+'site/members/quickSignup',
        data: {"email": email},
        dataType: 'json',
        success: function(response)
        {
...
function quickSignup2(){
	...
	$.ajax({
        type: 'POST',
        url: baseURL+'site/members/quickSignupUpdate',
...
function reload_captcha(){
		$.ajax({
	        type: 'POST',
	        url: baseURL+'site/members/reloadCaptcha',
	        success: function(data)
	        {
				data = $.trim(data);
				var captch = data.split('-');
				$('#captacha').val(captch[0]+captch[1]);
				$('#captacha1').html(captch[0]);
				$('#captacha2').html(captch[1]);
 ...
 function onlyAlphabets(e, t) {
            try {
                ...
                if ((charCode > 64 && charCode < 91) || (charCode > 96 && charCode < 123) || charCode == 8 || charCode == 46 || e.keyCode == 9)
                    return true;
                else
                    return false;
            }
...
function register_user(){
	...
	var cnf_pwd = $('#cnf_password').val();
	 var pwd_strength=$('#new_password_warn').html();
	if(firstname=='' || firstname=='First Name'){
		sweetAlert(Oops,Enter_first_name, "error");
	}else if(lastname=='' || lastname=='Last Name'){
		sweetAlert(Oops, Enter_last_name, "error");
	}else if(email=='' || email=='Email Address'){
		sweetAlert(Oops, Enter_email, "error");
	}else if(pwd==''){
		sweetAlert(Oops, Enter_pass, "error");
	}else if(pwd.length < 6){
		sweetAlert(Oops, Enter_pass_char, "error");
	}else if(pwd_strength=="Weak"){
		sweetAlert(Oops,weak_pass,'error');
	}else if(cnf_pwd==''){
		sweetAlert(Oops, pass_again, "error");
	}else if(pwd!=cnf_pwd){
		sweetAlert(Oops, Pass_not_match, "error");
	}else {
		var news_signup = 'no';
	...
	if($('#captacha').val() != $('#register_captcha').val())
	{
		$.ajax({
	        type: 'POST',
	        url: baseURL+'site/members/reloadCaptcha',
		...
		sweetAlert(Oops, EnterValidCaptcha, "error");
		$('#register_captcha').val('');
		...
		$.ajax({
	        type: 'POST',
	        url: baseURL+'site/members/registerUser',
...
function CheckPasswordStrength(password) {
        ...
        var password_strength = document.getElementById("new_password_warn");
...
function signin(){
	...
	//var next_url=$('#next_url').val();
	 ...
	 if(signin_email==''){
		sweetAlert(Oops, EnterEmailAddress, "error");
	}else if(signin_password==''){
		sweetAlert(Oops, EnterPassword, "error");
	}else if(signin_password.length < 6){
		sweetAlert(Oops, PasswordSixCharacters, "error");
	}
	...
	alert('Invalid Email Id');
		...
		$.ajax({
	        ...
	        type: 'POST',
	        url: baseURL+'site/members/login_user',
...
function paypaldetail(){
	...
	if(bank_name==''){
		sweetAlert("Oops...", "Bank full name required", "error");
	}else if(bank_no==''){
		sweetAlert("Oops...", "Bank number required", "error");
	}else if(bank_code==''){
		sweetAlert("Oops...", "Bank code required", "error");
	}else if(paypal_email==''){
		sweetAlert("Oops...", "Email id required", "error");
	}else{

		$.ajax({

	        type: 'POST',
	        url: baseURL+'site/members/paypaldetail',
...
function RentalEnquiry(){
	...
	//alert(caltophone);
	if(checkin==''){
		sweetAlert("Oops...", "Please select checkin date", "error");
		return false;
	}else if(checkout==''){
		sweetAlert("Oops...", "Please select checkout date", "error");
		return false;
	}else{
		...
		$.ajax({
	        ...
	        type: 'POST',
	        url: baseURL+'site/members/rentalEnquiry',
...
function forgot_password(){
	...
	var forgot_email = $('#forgot_email').val();
	 ...
	 if(forgot_email==''){
		...
		$('#load-img-forgot').css('display','none');
		return false;
	}
		$.ajax({
	        type: 'POST',
	        url: baseURL+'site/members/forgot_password_user',
...
function resendConfirmation(mail){
	if(mail != ''){
		...
		$.ajax({
	        type: 'POST',
	        url: baseURL+'site/members/resend_confirm_mail',
	        data: {"mail": mail},
	        dataType: 'json',
	        success: function(response){
	        	if(response.success == '0') {
...
function updateUserPhoto(){
	...
	if($('.uploadavatar').val()==''){
		sweetAlert("Oops...", "Choose a image to upload", "error");
...
function update_cart(val,cid) {
	...
	if(qty-qty != 0 || qty == '' || qty == '0'){
		...
		sweetAlert("Oops...", "Invalid quantity", "error");
		return false;
	}
	if(qty>mqty){
		$('#quantity'+cid).val(mqty);
		qty = mqty;
		sweetAlert("Oops...", "Maximum stock available for this product is "+mqty, "error");
	}
		$.ajax({
			type: 'POST',
...
function change_user_password(){
	...
	if(pwd == ''){
		sweetAlert("Oops...", "Enter new password", "error");
		...
		sweetAlert("Oops...", "Confirm password required", "error");
		$('#save_password').removeAttr('disabled');
		$('#confirmpass').focus();
		return false;
	}else if(pwd != cfmpwd){
		sweetAlert("Oops...", "Passwords doesnot match", "error");
		$('#save_password').removeAttr('disabled');
		$('#confirmpass').focus();
		return false;
	}else{
...
function changeShipStatus(value,dealCode,seller){
	...
	$.ajax({
		type:'post',
		url:baseURL+'site/members/change_order_status',
...
function changeCatPos(evt,catID){
	...
	if((pos-pos) != 0){
		sweetAlert("Oops...", "Invalid position", "error");
		return;
	}else{
		$(evt).hide();
		$(evt).next().show();
...
function post_order_comment(pid,utype,uid,dealcode){
	...
	if(uid==''){
		sweetAlert("Oops...", "Login required", "error");
		$('.order_comment_'+pid).removeClass('posting');
	}else{
		if($('.order_comment_'+pid).val() == ''){
			sweetAlert("Oops...", "Your comment is empty", "error");
			$('.order_comment_'+pid).removeClass('posting');
		}else{
			$form.find('img').show();
			...
			$.ajax({
				type:'post',
				url:baseURL+'site/members/post_order_comment',
...
function changeReceivedStatus(evt,rid){
	...
	$.ajax({
		type:'post',
		url:baseURL+'site/members/change_received_status',
...
function update_refund(evt,uid){
	...
	if(amt == '' || (amt-amt != 0)){
		sweetAlert("Oops...", "Enter valid amount", "error");
...
/* function DeleteWishList(pid,wid){
	...
	var res = window.confirm("<?php if($this->lang->line('sure_to_delete') != '') { echo stripslashes($this->lang->line('sure_to_delete')); } else echo 'Are you sure want to delete';?>?");
	if(res){
		var id = 'wish_'+pid;
		var sPath = window.location.pathname;
		var sPage = sPath.substring(sPath.lastIndexOf('/') + 1);
		$.ajax(
				{
					...
					success: function(json)
					{
						if(json.result == '0'){
						  $('#'+id).remove();
						  sweetAlert("Wow, Great!", "Wish list deleted successfully", "success");
						  window.location.reload();
						}
						if(json.result == '1'){
							$('#'+id).remove();
...
/*function DeleteallWishList(lid)
{
	...
	if(res){
		...
		$.ajax(
				{
					type: 'POST',
					url: BaseURL+'site/members/DeleteallWishList',
					data: {"lid": lid},
					dataType: 'json',
					success: function(response)
					{
...
function ChangePassword(){
	...
	if($('#old_password').val()==''){
		$('#old_password_warn').html(EnterCurrPassword);
		$('#old_password').focus();
		return false;
	}else if($('#new_password').val()==''){
		$('#new_password_warn1').html(EnterNewPassword);
		$('#new_password').focus();
		return false;
	}
	else if($("#new_password_warn1").html() == 'Weak'){
		$("#new_password_warn1").html(PasswordShouldCharNumber);
		$("#new_password").focus();
	    return false;						
	}
	else if($('#confirm_password').val()==''){
		$('#confirm_password_warn').html(EnterConfPassword);
		$('#confirm_password').focus();
		return false;
	}else if($('#confirm_password').val()!=$('#new_password').val()){
		$('#confirm_password_warn').html(PasswordNotMatch);
		$('#confirm_password').focus();
		return false;
	}else if($('#confirm_password').val()==$('#old_password').val()){
		$('#confirm_password_warn').html(OldNewNotSame);
		$('#confirm_password').focus();
		return false;
	}else if($('#new_password').val().length <= 5){
		$('#new_password_warn').html(MinimumCharacters);
		$('#new_password_warn1').html(MinimumCharacters);
		$('#new_password_warn1').css('color','red');
		$('#new_password').focus();
		return false;
	}else{
		$( "#changePassword1" ).submit();
	}
}
...
function delete_special_offer(bookingNo, id)
{
	$.ajax(
	{
		type: 'POST',
		url: baseURL+'site/product/delete_special_offer',
		data:{'bookingNo':bookingNo,'id':id},
		success: function(data) 
		{
			$('#delete_special_offer').remove();
			location.reload();
		}
	});
}

function offer_confirmation(action, bookingNo, id)
{
	$.ajax({
		type: 'POST',
		url: baseURL+'site/rentals/confirm_offer',
		data:{"action":action,"bookingNo":bookingNo,"id":id},
		success: function(data) 
		{
			/* $('#offer_accept_level_'+id).html(data); */
			location.reload();
		}
	});
}
